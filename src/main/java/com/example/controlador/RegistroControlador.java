...
public class RegistroControlador {
	...
	public String Login(@ModelAttribute Usuarios usuario, Model model, HttpSession sesion, RedirectAttributes redirect) {
		...
		if(u!=null) {
			String contrasenaEncriptada = new String(base64.encode(usuario.getClave().getBytes()));
			if(u.getClave().equals(contrasenaEncriptada)) {
	...
	public String Resgistarse(@ModelAttribute Usuarios registro, Model model, HttpSession sesion, RedirectAttributes redirect) {
		...
		if(u==null) {
			String claveEncriptada = new String(base64.encode(registro.getClave().getBytes()));         
	...
	public String muestraListado(Model model, HttpSession sesion) {
		...
		Usuarios este= (Usuarios)sesion.getAttribute("usuario");
		ArrayList<Usuarios> usus= new ArrayList<Usuarios>();
		if(p!=null)
		{
			for (Usuarios usuar : p) {
				if(usuar.getIdRol()>=este.getIdRol()) {
					usus.add(usuar);
				}
			}
		}
		model.addAttribute("usuarioss", usus);
	...
	@GetMapping("/editarUnoContrasena/{n}")
	public String editarContrasenaUno(@PathVariable("n") int id, Model model, HttpSession sesion) {
		Usuarios usuario= usu.findById(id);
		model.addAttribute("usu", usuario);
		return "muestrasyeliminaciones/editarUsuarioUnicoContrasena";
	}
	...
	@PostMapping("/edicionUnoContrasena")
	public String editarPerfilUnoContrasena(@ModelAttribute Usuarios registro, Model model, HttpSession sesion, RedirectAttributes redirect) {
		String contrasenaEncriptada = new String(base64.encode(registro.getClave().getBytes()));
		registro.setClave(contrasenaEncriptada);
		usu.save(registro);
		return "login/loginForm";
	}
	
	@GetMapping("/registro2")
	public String registrarAdmin(Model model, HttpSession sesion) {
		return "muestrasyeliminaciones/crearUsuariosAdmin";
	}
	
	@PostMapping("/registrarAdmin")
	public String resgistarAdmin(@ModelAttribute Usuarios registro, Model model, HttpSession sesion, RedirectAttributes redirect) {
		Usuarios u= usu.findByEmail(registro.getEmail());
