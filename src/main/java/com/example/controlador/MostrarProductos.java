...
public class MostrarProductos {
	...
	@GetMapping("/excel")
	public String excel(Model model, HttpSession sesion) {
		int i=0;
		ArrayList<Productos> producto= (ArrayList<Productos>)usu.DarProducto();
		Configuracion config = conf.findByClave("HojaExcel");
		String nombre = "ficheroExcel/"+config.getValor()+"productos.xls";
		File fichero = new File(nombre);
		try {
			
			WritableWorkbook excels = Workbook.createWorkbook(fichero);
			//Nombre de la hoja
			WritableSheet hoja= excels.createSheet("Productos", 0);
			
			jxl.write.Label cadena4 = new jxl.write.Label(0, i, "nombre");
			hoja.addCell(cadena4);
			jxl.write.Label cadena5 = new jxl.write.Label(1, i, "Descripción");
			hoja.addCell(cadena5);
			jxl.write.Label cadena6 = new jxl.write.Label(2, i, "Precio");
			hoja.addCell(cadena6);
			jxl.write.Label cadena7 = new jxl.write.Label(3, i, "Strock");
			hoja.addCell(cadena7);
			
			i++;
			//Ahora pasaremos a intoducir los datos
			for (Productos p : producto) {
				jxl.write.Label cadena = new jxl.write.Label(0, i, p.getNombre());
				hoja.addCell(cadena);
				jxl.write.Label cadena1 = new jxl.write.Label(1, i, p.getDescripcion());
				hoja.addCell(cadena1);
				jxl.write.Number cadena2 = new jxl.write.Number(2, i, p.getPrecio());
				hoja.addCell(cadena2);
				jxl.write.Number cadena3 = new jxl.write.Number(3, i, p.getStock());
				hoja.addCell(cadena3);
				i++;
				
			}
			
			//Y por ultimo guardar estos
			excels.write();
			excels.close();
		...
		int valor = Integer.parseInt(config.getValor())+1;
		String numero = valor+"";
